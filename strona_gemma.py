...
st.title("Lupa na prompt - Analiza wyników Gemma")
...
def load_data():
    try:
        parsed_df = pd.read_csv("saved_responses/parsed_responses.csv", index_col=0)
        raw_df = pd.read_csv("saved_responses/raw_responses.csv", index_col=0)

        df = pd.read_csv('saved_responses/prompts_made.csv', index_col=0)
        return parsed_df, raw_df, df
    except FileNotFoundError:
        st.error("Brak plików wyników. Uruchom najpierw gemini.ipynb")
        st.stop()
...
with tab6:
    ...
    st.info("Dostępne pliki będą wyeksportowane do folderu results_gemini/")
